...
final class SubscriptionManager: ObservableObject {
    ...
    private func setupApphud() {
        ...
        print("🚀 SubscriptionManager: Apphud delegate configured")
            ...
            DispatchQueue.main.async {
                ...
                if let paywallID = self?.paywallID,
                   let found = placements.first(where: { $0.identifier == paywallID }) {
                    ...
                    if placement != nil {
                        print("⚠️ Paywall с ID \(self?.paywallID ?? "main") not found, use the first available one.")
                    }
                }
                ...
                if let paywall = placement?.paywall {
                    ...
                    print("✅ Products loaded: \(paywall.products.count)")
                } else {
